...
pub fn init(com_port: u16, baud_rate: u16) Serial {
...
pub fn terminal(self: *Serial) Terminal {
    return Terminal.init(self, read, write);
}

pub fn read(self: *Serial) u8 {
    while ((port.inb(self.com_port + LINE_STATUS_REGISTER) & 0x01) == 0x00) {
        // Wait for data to be received
    }

    return port.inb(self.com_port + DATA_REGISTER);
}

pub fn write(self: *Serial, char: u8) void {
    while ((port.inb(self.com_port + LINE_STATUS_REGISTER) & 0x20) == 0x00) {
        // Wait for empty transmit buffer
    }

    port.outb(self.com_port + DATA_REGISTER, char);
}
