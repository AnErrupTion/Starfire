...
.PHONY: run
run: $(IMAGE_NAME).iso
	qemu-system-x86_64 -enable-kvm -M q35 -m 2G -cdrom $(IMAGE_NAME).iso -s -S -boot d -display none -serial stdio

.PHONY: run-uefi
run-uefi: ovmf $(IMAGE_NAME).iso
	qemu-system-x86_64 -enable-kvm -M q35 -m 2G -bios ovmf/OVMF.fd -cdrom $(IMAGE_NAME).iso -boot d -display none -serial stdio

.PHONY: run-hdd
run-hdd: $(IMAGE_NAME).hdd
	qemu-system-x86_64 -enable-kvm -M q35 -m 2G -hda $(IMAGE_NAME).hdd -display none -serial stdio

.PHONY: run-hdd-uefi
run-hdd-uefi: ovmf $(IMAGE_NAME).hdd
	qemu-system-x86_64 -enable-kvm -M q35 -m 2G -bios ovmf/OVMF.fd -hda $(IMAGE_NAME).hdd -display none -serial stdio
